...
The Solr MCP server currently supports only keyword (BM25) search via its `search` tool. Modern search applications benefit from semantic understanding — finding documents by meaning, not just keyword overlap. Apache Solr 9.0+ introduced native Dense Vector Search (KNN/HNSW), and Solr 10 continues to support it. This spec defines a `SolrVectorStore` (implementing Spring AI's `VectorStore` interface) as the foundation, plus three new MCP tools that add semantic search, hybrid search, and embedding-aware indexing to the MCP server. These features target the **2.0.0 release**, built on Spring Boot 4 and Spring AI 2.0.
...
Collections used with semantic/hybrid search must have a `DenseVectorField` configured in their schema. `DenseVectorField` is supported on Solr 9.0+ and Solr 10. Example schema additions (via Solr Schema API):
...
Users must add the corresponding Spring AI 2.0 starter dependency to their classpath (e.g., `spring-ai-starter-openai`). Note the starter naming convention changed in Spring AI 2.0: `spring-ai-openai-spring-boot-starter` → `spring-ai-starter-openai`.
...
Solr KNN query parser syntax (Solr 9.0+ / 10):
```
...
### Integration Tests
- Testcontainers Solr 10 with Schema API setup (low-dimensional vectors, e.g. 3D, for speed)
...
- **Solr 9.0+** required for DenseVectorField/KNN support; **Solr 10** is the primary target for 2.0.0
- **Solr 8.x** — semantic/hybrid tools will fail at query time (Solr error); keyword search unaffected
- **Spring Boot 4.0.2**, **Spring AI 2.0.0-M2** — target stack for 2.0.0 release
- **SolrJ 10.0.0** — used for all Solr client interactions
- **Java 25+** — no additional requirements beyond the base project
...
|---|---|---|
| `UrlIndexingService` | `@Service @Observed` | The `@McpTool` method; wires `HttpClient`, `UrlTargetPolicy`, `UrlFetcher`; error mapping (§4.5). Constructor takes `IndexingService` and `UrlIndexingProperties`. **No** `@Profile`, **no** `@ConditionalOnNotWebApplication`. Tool method carries `@PreAuthorize("isAuthenticated()")`. |
...
| `UrlIndexingServiceTest` | unit, Mockito, `@DisabledInNativeImage` | error mapping table (§4.5) row by row using a mocked `IndexingService` and a mocked `UrlFetcher`; explicit `format` overrides extension; extension overrides media type; `?query` is ignored for extension. (Header assertions live in the integration test, where the real request can be inspected.) |
...
| `McpClientIntegrationTest` (HTTP) | integration, existing | **the transport-parity guard**: `tools/list` contains `index-url` and not `index-file`; one `index-url` round trip against the test `HttpServer` verifies counts via `search`. |
| `McpClientStdioIntegrationTest` | integration, existing | `tools/list` contains **both** `index-url` and `index-file`; one `index-url` round trip alongside the existing `index-file` ones. |

...
**Amended 2026-09-24** for #205 and #202, which merged after this spec was written:
CSV and XML are no longer parsed by the server but forwarded to Solr's own update
...
**D3. The body is read into memory, capped at `max-bytes` (default 10 MB), and indexed exactly as the inline tool for its format would index it.**
*Why:* it reuses code that is already tested and adds no parser code: CSV and XML go
to Solr's own update handlers as `index-csv-documents` and `index-xml-documents` send
them (so XML must be a Solr `<add>` block), while JSON and Markdown are parsed by the
server's creators first. A fetch failure, or a JSON/Markdown parse failure, indexes
nothing; CSV and XML are accepted or rejected by Solr as with the inline tools. 10 MB is roughly 15,000 shows-sized JSON records, far
...
> Non-2xx responses and HTML pages are errors. Each format is indexed as its inline
> tool would: CSV and XML (an <add> block, as for index-xml-documents) go to Solr's own
> update handler, JSON and Markdown are parsed first. Reuse the URL for another
> collection.
...
### 4.4 `IndexingService.indexPayload` (as amended 2026-09-24)

`indexPayload` dispatches to the inline tools and leaves them unchanged: `"csv"` →
`indexCsvDocuments`, `"xml"` → `indexXmlDocuments`, `"markdown"` →
`indexMarkdownDocuments`, and `"json"` → `createSchemalessDocumentsFromJson(String)`
followed by the same indexing and summary as `indexJsonDocuments` (shared through a
private helper). The return value is therefore whatever the inline tool returns: a
document count for JSON and Markdown, Solr's acceptance for CSV and XML. The original
text below described the pre-#205 refactor and is kept for the record.

#### 4.4.1 Original design (superseded)
...
| `SolrException` with code 400 from `indexPayload` (CSV or XML that Solr cannot parse) | IAE | `Solr rejected the URL content as <format>: <Solr's message>` |
| `SolrServerException`, any other `SolrException` or `IOException` from `indexPayload` | ISE | `Solr could not complete URL indexing. Check collection availability and field types with get-schema, then verify the indexed count before retrying; some documents may already be indexed.` |
...
#197 (why one tool), and #205 (which this PR builds on: CSV and XML reach Solr's
update handlers through the inline tools, unchanged). It states the two
...
holding it in memory, which is what would lift the size cap.

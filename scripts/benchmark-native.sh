...
# ready signal on stdout, so we sample process memory until RSS stabilizes (two
# consecutive samples within 1 MB of each other), which is a reasonable proxy
# for end-of-startup on both JVM and native.
run_one() {
	local image="$1"
	local cid
	local t_start t_ready rss_mb prev_rss=0
	t_start="$(date +%s%N)"
	cid="$(docker run -d --rm \
		-e SOLR_URL="${SOLR_URL}" \

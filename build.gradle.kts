...
// `nativeTest` tasks. The plugin is only applied when -Pnative is set, so all
// graalvmNative DSL access must be guarded by `if (nativeBuild)`.
//
// AOT runs with the stdio or http profile per `-Pprofile=...`. Spring AOT bakes
// `spring.main.web-application-type` into the binary at AOT time, so activating
// both profiles picks `servlet` (http overrides stdio) and forces Tomcat to
// start regardless of runtime PROFILES — breaking stdio. Hence one native
// image per transport profile.
//
// Tests incompatible with native image are annotated @DisabledInNativeImage:
//   - Mockito-based tests (ByteBuddy cannot generate classes at run time)
//   - SolrJ indexing/query integration tests (response parsing uses reflection
//     that lacks native-image metadata)
// Collection management, schema, and observability integration tests run
// normally in the native binary.
if (nativeBuild) {

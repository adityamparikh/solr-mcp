...
//
// AOT runs with the active profile baked in. Spring AOT writes
// spring.main.web-application-type from the AOT-time profile into the binary,
// so one native image is produced per profile (stdio or http) — see
// `nativeProfile` above.
if (nativeBuild) {
